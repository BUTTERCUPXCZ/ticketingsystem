...
class AdminDashboard:
    ...
    def _configure_styles(self):
        ...
        style = ttk.Style()
        style.configure("Admin.TFrame", background="#e3f2fd",
                        bordercolor="#bbdefb")
        style.configure("Client.TFrame", background="#f5f5f5",
                        bordercolor="#e0e0e0")
        style.configure("Admin.TLabel", background="#e3f2fd",
                        foreground="#0d47a1")
        style.configure("Client.TLabel", background="#f5f5f5",
                        foreground="#4a148c")
    ...
    def respond_to_ticket(self, ticket_id, response_text=None):
        ...
        ttk.Label(info_frame, text=f"Ticket #{ticket_id}", font=(
            'Segoe UI', 12, 'bold')).pack(side=tk.LEFT)
        ttk.Label(info_frame, text=f"Client: {ticket['client']}", font=(
            'Segoe UI', 10)).pack(side=tk.LEFT, padx=10)
        ...
        scrollbar = ttk.Scrollbar(
            conv_frame, orient=tk.VERTICAL, command=canvas.yview)
        ...
        response_entry = tk.Text(
            response_frame, height=5, font=('Segoe UI', 10))
    ...
    def close_ticket(self, ticket_id):
        ...
        try:
            ...
            save_data(data)
            messagebox.showinfo(
                "Success", f"Ticket #{ticket_id} has been closed!")
            self.show_all_tickets()
        except Exception as e:
